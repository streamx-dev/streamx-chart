...
StreamX supports multi-tenancy. We recommend to create a separate namespace for each tenant. The namespace is used to isolate the services and their data. The tenant is configured via `tenant` value. The default value is the release name.
...
Apache Pulsar supports multi-tenancy, and StreamX supports it as well. We recommend to create a separate namespace for each tenant. Then, the tenant initialization has to be performed in each tenant's namespace (for each tenant). It guarantees that the tenant's data is isolated from other tenants at both the Kubernetes and Apache Pulsar levels (even a single Apache Pulsar installation is used).
...
1. Create secret for Rest Ingrestion Auth:
   ...
   kubectl apply -f examples/reference/secrets/tenant-1-rest-ingestion-jwt-keys.yaml
   ```
   ...
   openssl rsa -pubout -in rsaPrivateKey.pem -out publicKey.pub
   openssl pkcs8 -topk8 -nocrypt -inform pem -in rsaPrivateKey.pem -outform pem -out privateKey.pem
   kubectl create namespace tenant-1 || true
   kubectl -n tenant-1 create secret generic streamx-api-keys --from-file=./privateKey.pem --from-file=./publicKey.pub
   ```
   </p>
   </details>
